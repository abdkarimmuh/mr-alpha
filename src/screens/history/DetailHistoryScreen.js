...
import ActionReturn from "./container/ActionReturn"
...
export default class DetailHistoryScreen extends Component {
    ...
    componentDidMount() {
        this.setState({ data: JSON.parse(this.props.navigation.getParam("data")) })
    }
                            ...
                            {data.map((data, index) => (
                                <View key={index}>
                                    <View style={styles.containerRowStatus}>
                                        ...
                                        <Text style={[styles.viewStatusQty, { fontWeight: "bold", fontSize: 14 }, this.getColorStatus(data.status)]}>{data.qty_minus} {data.uom_assign}</Text>

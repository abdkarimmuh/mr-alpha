...
import { Container, TextInput, Button, HelperText, Loading } from "@app/components"
...
class ChangePasswordScreen extends PureComponent<Props> {
    ...
    pressSubmit = async () => {
        ...
        if (this.state.new_password == this.state.confirm_password) {
            Api.post()
                .changePassword(this.props.token, this.state.old_password, this.state.new_password)
                .then(res => {
        ...
        return (
            <Container style={{ backgroundColor: Color.backgroudDefault, flex: 1 }}>
                ...
                <View style={styles.containerBtnButtom}>
                    ...
                    <Button mode="contained" onPress={() => this.pressSubmit()} disabled={this.state.isFetching}>SAVE</Button>
                </View>
            </Container>
        )
    }
